...
<div class="bg-gray-200 p-4 rounded grid grid-cols-2 gap-4 mb-6">
    ...
    {#if total.gst18 > 0}
        <div class="font-bold">{useIGST ? "18% IGST" : "9% CGST"}</div>
        <div class="text-right">{(total.gst18 / 2).toFixed(2)}</div>

        {#if !useIGST}
            <div class="font-bold">9% SGST</div>
            <div class="text-right">{(total.gst18 / 2).toFixed(2)}</div>
        {/if}
    {/if}

    {#if total.gst28 > 0}
        <div class="font-bold">{useIGST ? "28% IGST" : "14% CGST"}</div>
        <div class="text-right">{(total.gst28 / 2).toFixed(2)}</div>

        {#if !useIGST}
            <div class="font-bold">14% SGST</div>
            <div class="text-right">{(total.gst28 / 2).toFixed(2)}</div>
        {/if}
    {/if}

    {#if total.gst12 > 0}
        <div class="font-bold">{useIGST ? "12% IGST" : "6% CGST"}</div>
        <div class="text-right">{(total.gst12 / 2).toFixed(2)}</div>

        {#if !useIGST}
            <div class="font-bold">6% SGST</div>
            <div class="text-right">{(total.gst12 / 2).toFixed(2)}</div>
        {/if}
    {/if}
